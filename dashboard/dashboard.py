...
<div class="header-container">
    <div style="display:flex; justify-content:space-between; align-items:center;">
        ...
        <div style="text-align:right;">
            ...
            <p style="margin:4px 0 0 0; color:#475569; font-size:12px;">IEEE-CIS Dataset · 100K Transactions</p>
        </div>
    </div>
</div>
""", unsafe_allow_html=True)
...
with left:
    ...
    <div style="background:#0d1b2e; border:1px solid #1e3a5f; border-radius:10px; padding:14px;">
        <table style="width:100%; color:#94a3b8; font-size:12px; border-collapse:collapse;">
            ...
            <tr><td style="padding:4px 0;">Training Size</td><td style="color:white; text-align:right;">100,000 txns</td></tr>
...
with right:
    if predict_btn:
        ...
        try:
            ...
            if prediction == "FRAUD":
                st.markdown(
                    '<div class="fraud-verdict">⚠️ FRAUDULENT TRANSACTION DETECTED</div>',
                    unsafe_allow_html=True
                )
            else:
                st.markdown(
                    '<div class="legit-verdict">✅ LEGITIMATE TRANSACTION</div>',
                    unsafe_allow_html=True
                )
            
            m1, m2, m3, m4 = st.columns(4)
            with m1:
                st.metric("Fraud Risk", f"{fraud_prob*100:.1f}%")
            with m2:
                st.metric("Amount", f"${amount:,.0f}")
            with m3:
                st.markdown(f'<br><span class="{alert_class}">{alert_emoji} {alert}</span>',
                           unsafe_allow_html=True)
            with m4:
                st.metric("Response", f"{response_time:.0f}ms")
            ...
            if prediction == "FRAUD":
                ...
                <div style="background:#450a0a; border:1px solid #ef4444;
                    ...
                    ⚠️ Why this transaction was flagged:</p>
                    ...
                    • Amount ${amount:,.2f} analyzed against 100K historical transactions
                    </p>
                </div>
                """, unsafe_allow_html=True)
            else:
        ...
        <div style="background:#0d1b2e; border:1px solid #1e3a5f; border-radius:16px;
            ...
            👈 Enter transaction details to begin analysis</h2>
        ...
        steps = [
            ("1️⃣", "Input", "Enter transaction amount and card behavior metrics"),
            ("2️⃣", "GNN", "GraphSAGE analyzes 13,553 node transaction graph"),
            ("3️⃣", "ML Model", "Random Forest predicts fraud probability"),
            ("4️⃣", "Decision", "Instant verdict with full risk breakdown")
        ]
        for col, (emoji, title, desc) in zip([h1, h2, h3, h4], steps):
            with col:
                st.markdown(f"""
...
if st.session_state.history:
    ...
    if len(st.session_state.history) > 1:
        ...
        fig_trend.update_layout(
            title=dict(text='Fraud Risk Trend — This Session',
